...
# Change directory to the e2xgrader_restricted_notebook_extension directory
...
```bash
pip uninstall e2xgrader_restricted_notebook_extension
```

In development mode, you will also need to remove the symlink created by `jupyter-builder develop`
command. To find its location, you can run `jupyter labextension list` to figure out where the `labextensions`
folder is located. Then you can remove the symlink named `@e2xgrader/restricted-notebook-extension` within that folder.
